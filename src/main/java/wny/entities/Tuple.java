...
public class Tuple implements Comparable<Tuple>
{
    ...
    */
    public Tuple(List<Tuple> list_of_tuples)
    {
     ...
     * @return An integer showing whether the tuple has larger, same or smaller score.
     */
    @Override
     ...
     */
    public int isDominating(Tuple other, double gap)
    {
        int count = 0;
        for (int i = 1; i < values.length; i++) {
            if (Double.valueOf(this.values[i]) >= Double.valueOf(other.values[i]) - gap) {
                count++;
            } else if (Double.valueOf(this.values[i]) < Double.valueOf(other.values[i]) - gap) {
                count--;
            }
        }
